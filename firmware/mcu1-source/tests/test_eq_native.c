...
int main(void)
{
    ...
    {   /* Live apply; discard restores the saved profile AND re-entry shows it. */
        unsigned base=128;
        assert(io.write(base+OMNI_EQ_FLAT,0));                 /* flat custom draft */
        assert(io.write(base+5,200));                         /* band5 -> +8.0 dB */
        assert(io.write(base+OMNI_EQ_APPLY,0));complete(true); /* save: baseline := this */
        assert(io.write(base+OMNI_EQ_BEGIN,0));
        assert(io.read(base+5,&value) && value==200);
        assert(io.write(base+5,120));                         /* live edit band5 -> 0 dB */
        omni_settings_menu_live_poll(now);complete(true);
        assert(wire[2]==0x1bu && wire[4+68+6*5+3]==0u);        /* previewed 0 dB */
        assert(io.write(base+OMNI_EQ_DISCARD,0));
        omni_settings_menu_live_poll(now);complete(true);
        assert(wire[4+68+6*5+3]==80u);                         /* restored +8.0 dB */
        assert(io.write(base+OMNI_EQ_BEGIN,0));                /* re-enter editor */
        assert(io.read(base+5,&value) && value==200);          /* saved, not discarded */
    }
